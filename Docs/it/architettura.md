...
La struttura root contiene i moduli di sistema; i dati di progetto sono isolati nel modello Workspace:
...
└── Workspace/              Dati dei progetti (creato automaticamente)
    └── projects/
        └── {nome_progetto}/
            ├── project_info.json                    Metadati progetto e tracce audio
            ├── audio_extraction/
            │   ├── current/                         Tracce audio correnti (track_01.wav, ecc.)
            │   └── archive/                         Archivio elaborazioni precedenti
            ├── transcripts/
            │   ├── current/                         Trascrizioni SRT correnti
            │   └── archive/                         Archivio trascrizioni precedenti
            ├── translated/
            │   ├── current/                         Traduzioni SRT correnti
            │   └── archive/                         Archivio traduzioni precedenti
            ├── dubbed/
            │   ├── current/                         Audio TTS corrente
            │   └── archive/                         Archivio audio TTS precedente
            ├── video_input/                         Video sorgente (mai modificati)
            └── audio_input/                         Audio di input diretto (opzionale)
```
...
|---|---|---|
| 1 — Estrazione audio | `Scripts/estrai_tracce.py` | `video_input/` → `audio_extraction/current/` (track_01.wav, ecc.) |
| 2 — Trascrizione | `Scripts/trascrivi_audio.py` | `audio_extraction/current/` o `audio_input/` → `transcripts/current/` (SRT) |
| 3 — Traduzione | `Scripts/traduci_testo.py` | `transcripts/current/` → `translated/current/` (SRT) |
| 4 — TTS | `Scripts/tts_menu.py` | `translated/current/` → `dubbed/current/` (MP3/WAV) |

Tutti i percorsi sono relativi a `Workspace/projects/{nome_progetto}/`. `tts_menu.py` delega a `tts_azure.py` o `tts_google.py` in base al provider attivo.

I file elaborati ad ogni fase vengono archiviati automaticamente in cartelle `archive/` per preservare la cronologia.
...
### `estrai_tracce.py`
Estrazione tracce audio dai video tramite ffmpeg. Salva i file audio in `audio_extraction/current/` con nomi `track_01.wav`, `track_02.wav`, ecc. Genera automaticamente file di metadati (`track_XX_metadata.json`) e archivia le elaborazioni precedenti in `audio_extraction/archive/`.

### `trascrivi_audio.py`
Trascrizione audio tramite Whisper (o WhisperX, quando integrato). Output in formato SRT in `transcripts/current/`. I file precedenti sono archiviati in `transcripts/archive/`.

### `traduci_testo.py`
Traduzione SRT tramite modelli Helsinki-NLP MarianMT in esecuzione locale. Output in `translated/current/`. I file precedenti sono archiviati in `translated/archive/`.
...
Backend TTS specifici per provider. Generano i segmenti audio e li salvano in `dubbed/current/`. I file precedenti sono archiviati in `dubbed/archive/`.
...
### `info_manager.py`
Lettura e scrittura del file `project_info.json` nella root del progetto. Centralizza metadati di progetto, elenco tracce audio estratte (`audio_tracce[]`), stato della pipeline e cronologia elaborazioni. Garantisce la tracciabilità dello stato tra le fasi.
...
| `Protect-Placeholders.ps1` | Protezione placeholder durante la traduzione automatica |
...
| File | Scopo |
|---|---|
| `build_include.json` | Cosa copiare, dove e in che modalità |
| `build_exclude.json` | Blacklist globale (tutte le modalità) |
| `build_exclude_test.json` | Blacklist aggiuntiva solo in modalità TEST (Python runtime, ffmpeg, voices) |
| `build_protected.json` | Percorsi copiati verbatim — le regole di esclusione vengono ignorate |
| `build_empty_dirs.json` | Cartelle vuote da creare nel pacchetto |

Parametri disponibili:
```powershell
.\build.ps1              # menu interattivo (scelta 1/2/3)
.\build.ps1 -Test        # build leggera senza componenti pesanti
.\build.ps1 -Production  # build completa con conferma
.\build.ps1 -DryRun      # simulazione senza scrivere file
```
...
Ogni fase della pipeline genera file nella sottocartella `current/` del relativo stage (es. `audio_extraction/current/`, `transcripts/current/`, ecc.). Quando viene avviata una nuova elaborazione nello stesso stage, i file precedenti in `current/` vengono automaticamente archiviati in `archive/` con timestamp, preservando la cronologia.

Alcuni stage generano file di metadati (es. `track_XX_metadata.json`, `_info.txt`) per tracciare le proprietà dell'elaborazione.
...
| Modello project-based (Workspace con cartella per progetto) | ✅ Operativo |